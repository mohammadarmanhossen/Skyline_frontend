...
const displayHotels = (hotelsData) => {
  ...
  if (hotelsData.length === 0) {
    container.innerHTML =
      '<p class="text-center text-gray-500">No hotels available.</p>';
    return;
  }
    ...
    .map(
      (hotel) => `
        <div class="max-w-sm bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden m-4">
            ...
            <div class="px-6 py-4">
                <h5 class="text-2xl font-semibold text-gray-700 mb-1">${getShortName(
                  hotel.hotel_name
                )}</h5>
                <p class="text-gray-700 text-sm mb-3">${getShortDescription(
                  hotel.description
                )}</p>
                <p class="text-gray-700 text-sm mb-3">Room : ${
                  hotel.available_room
                }</p>
               <p class="text-gray-700 text-sm mb-3">District : ${
                 hotel.district_name
               }</p>
              
               <p class="text-gray-700 text-sm mb-3">Location : ${
                 hotel.address
               }</p>
                <a href="./hotel_details.html?id=${
                  hotel.id
                }" class="block rounded-lg">
                    <div class="flex justify-between items-center bg-white  transition-colors duration-200">
                     <span><span class="text-black font-seibold">৳${
